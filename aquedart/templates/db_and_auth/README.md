...
You will need a local database for testing, and another database for running the application locally. The behavior and configuration of these databases are documented here: http://aldrinsartfactory.github.io/aquedart/testing/mixins/.
...
aquedart db upgrade --connect postgres://dart:dart@localhost:5432/wildfire
```
...
```
aquedart auth add-client --id com.local.test \
    --secret mysecret \
    --connect postgres://user:password@localhost:5432/wildfire
```

To run your tests with OAuth 2.0 client identifiers, see this documentation: http://aldrinsartfactory.github.io/aquedart/testing/mixins/#testing-applications-that-use-oauth-20.

## Running the server locally

Run `aquedart serve` from this directory to run the application. For running within an IDE, run `bin/main.dart`.
...
Endpoints that are not associated with a user must have client authentication using the credentials added with `aquedart auth`. These routes are `POST /register` and `POST /auth/token`.
...
To generate a SwaggerUI client, run `aquedart document client`.
...
See the documentation for [Deployment](https://aldrinsartfactory.github.io/aquedart/deploy/).
