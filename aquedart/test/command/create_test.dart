  ...
  group("Templates", () {
    ...
    test("Template gets generated from local path, project points to it",
      ...
      var aquedartLocationString = File.fromUri(cli
          ...
          .readAsStringSync()
          .split("\n")
          .firstWhere((p) => p.startsWith("aquedart:"))
          .split("aquedart:")
          .last;

      var path = path_lib.normalize(path_lib.fromUri(aquedartLocationString));
    ...
    final templates = Directory("templates")
        ...
        .toList();
    final aquedartPubspec = loadYaml(File("pubspec.yaml").readAsStringSync());
    final aquedartVersion = Version.parse("${aquedartPubspec["version"]}");

    for (var template in templates) {
      test("Templates can use 'this' version of aquedart in their dependencies",
          () {
        ...
        final projectVersionConstraint = VersionConstraint.parse(
            contents["dependencies"]["aquedart"] as String);
        expect(projectVersionConstraint.allows(aquedartVersion), true);
      });
