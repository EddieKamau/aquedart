...
/// The `aquedart db` tool will create database tables for these types as long as they are visible
/// to the application's library file, as noted above.
library aquedart.managed_auth;
...
class ManagedAuthToken extends ManagedObject<_ManagedAuthToken>
  ...
  ManagedAuthToken.fromToken(AuthToken t) : super() {
    final tokenResourceOwner = entity
        .relationships!["resourceOwner"]!.destinationEntity!
        .instanceOf()!;
...
/// code flow.
///
/// Use the `aquedart auth` tool to add new clients to an application.
...
class ManagedAuthDelegate<T extends ManagedAuthResourceOwner>
  ...
  FutureOr<AuthToken?> getToken(AuthServer server,
    ...
    ManagedAuthToken? token = await query.fetchOne();

    return token?.asToken();
  }

  @override
  FutureOr<T?> getResourceOwner(AuthServer server, String username) {
