...
abstract class CLICommand {
  ...
  Future<int> process(args.ArgResults results,
    ...
    try {
      ...
      if (showVersion) {
        outputSink.writeln("aquedart CLI version: $toolVersion");
        return 0;
      }

      if (!isMachineOutput) {
        displayInfo("aquedart CLI Version: $toolVersion");
      }
  ...
  Future determineToolVersion() async {
    try {
      var toolLibraryFilePath = (await Isolate.resolvePackageUri(
              currentMirrorSystem().findLibrary(#aquedart).uri))!
          .toFilePath(windows: Platform.isWindows);
      var aquedartDirectory = Directory(FileSystemEntity.parentOf(
          FileSystemEntity.parentOf(toolLibraryFilePath)));
      var toolPubspecFile =
          File.fromUri(aquedartDirectory.absolute.uri.resolve("pubspec.yaml"));
