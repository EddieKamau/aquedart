...
import 'package:aquedart/src/cli/metadata.dart';
...
/// Used internally.
class CLITemplateCreator extends CLICommand with CLIaquedartGlobal {
  CLITemplateCreator() {
  ...
  Future<int> handle() async {
    if (projectName == null) {
      printHelp(parentCommandName: "aquedart");
      return 1;
    }
    ...
    displayProgress("See more templates with 'aquedart create list-templates'");
    copyProjectFiles(destDirectory, templateSourceDirectory, projectName);

    createProjectSpecificFiles(destDirectory.path);
    if (aquedartPackageRef!.sourceType == "path") {
      final aquedartLocation = aquedartPackageRef!.resolve()!.location;

      addDependencyOverridesToPackage(destDirectory.path, {
        "aquedart": aquedartLocation.uri,
        "aquedart_test": aquedartLocation.parent.uri.resolve("aquedart_test/")
      });
    }
  ...
  String get detailedDescription {
    return "This command will use a template from the aquedart package determined by either "
  ...
  String get description {
    return "Creates aquedart applications from templates.";
  }
}

class CLITemplateList extends CLICommand with CLIaquedartGlobal {
  @override
  Future<int> handle() async {
  ...
  @override
  String get description {
    return "List aquedart application templates.";
  }
...
class CLIaquedartGlobal {
  PubCache pub = PubCache();

  PackageRef? get aquedartPackageRef {
    return pub
        .getGlobalApplications()
        .firstWhere((app) => app.name == "aquedart")
        .getDefiningPackageRef();
  }

  Uri get templateDirectory {
    return aquedartPackageRef!.resolve()!.location.uri.resolve("templates/");
  }
