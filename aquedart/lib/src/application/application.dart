...
/// It is unlikely that you need to use this class directly - the `aquedart serve` command creates an application object
/// on your behalf.
class Application<T extends ApplicationChannel?> {
  ...
  ///
  /// This logger's name will appear as 'aquedart'.
  Logger logger = Logger("aquedart");
  ...
  Future start({int numberOfInstances = 1, bool consoleLogging = false}) async {
    if (server != null || supervisors.isNotEmpty) {
      throw StateError(
          "Application error. Cannot invoke 'start' on already running aquedart application.");
    }
  ...
  ///
  /// This method is called by the `aquedart document` CLI.
