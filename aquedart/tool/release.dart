...
class Runner {
  ...
  Uri baseReferenceURL = Uri.parse("https://www.dartdocs.org/documentation/aquedart/latest/");
  ...
  Future<Directory> directoryWithBranch(String branchName) async {
    ...
    var process = await Process.start(
        "git",
        ["clone", "-b", branchName, "git@github.com:stablekernel/aquedart.git", dir.path]);
    // ignore: unawaited_futures
    stderr.addStream(process.stderr);
    // ignore: unawaited_futures
    // stdout.addStream(process.stdout);
  ...
  Future<String> latestVersion() async {
    ...
    var response = await http.get(
        Uri.parse("https://api.github.com/repos/stablekernel/aquedart/releases/latest"),
  ...
  Future postGithubRelease(String? version, String? name, String? description) async {
    ...
    if (!isDryRun!) {
      var response = await http.post(Uri.parse("https://api.github.com/repos/stablekernel/aquedart/releases"),
        headers: {
  ...
  Future transformDirectory(List<Transformer> transformers, Directory source, Directory destination) async {
    ...
    for (var subdirectory in subdirectories) {
      ...
      Directory? destinationDir = Directory.fromUri(destination.uri.resolve("$dirName"));
